...
def make_continues(jets, mask,pt_bins,eta_bins,phi_bins, noise=False):
    ...
    #eta_con_j = np.sum(eta_con, -1)/200
    #phi_con_j = np.sum(phi_con, -1)/200

    #eta_con_j = np.max(eta_con, -1)
    #phi_con_j = np.max(phi_con, -1)
    #phi_con_j = phi_con_j[:, np.newaxis]
    #eta_con_j = eta_con_j[:, np.newaxis]
    #print(phi_con)
    #phi_con=np.add(phi_con,phi_con_j)
    #eta_con=np.add(eta_con,eta_con_j)
    #print(phi_con)
...
def Make_Plots(jets_gen,mj_gen,pt_bins,eta_bins,phi_bins,jets_true,ptj_true,mj_true,path_to_plots,plot_title):
    ...
    plt.hist(mask_phi_gen, bins=phi_bins, color='black',histtype='step',density=True,label="rot")
    ...
    plt.hist(np.clip(mj_gen, mj_bins[0], mj_bins[-1]), bins=mj_bins,color='black',histtype='step',density=True,label="Rot")
...
def LoadTrueRot(discrete_truedata_filename,n_samples):
    ...
    mask = tmp[:, :, 0] == -1
    print(mask)
    jets_true_rot,ptj_true_rot,mj_true_rot = make_continues_rot(tmp, mask,pt_bins,eta_bins,phi_bins, noise=False)

    return jets_true_rot,ptj_true_rot,mj_true_rot
...
for jet_name in list_of_jets:
    ...
    for gen_dir in gen_dirs:
        ...
        if 'ZJetsToNuNu_run_scan_10M_N1G96CW' not in gen_dir:
            continue
    
    
    
        discrete_truedata_filename=dir_of_test_data+jet_name+'_test___40_30_30_pt_part_'+jet_name+'.h5'
        #plot_title="$g/q $"
        plot_title=dict_of_names.get(jet_name)

        jets_true,ptj_true,mj_true=LoadTrue(discrete_truedata_filename,n_samples)
        jets_true_rot,ptj_true_rot,mj_true_rot=LoadTrueRot(discrete_truedata_filename,n_samples)
        ...
        for trunc in trunc_list:
            ...
            path_to_plots=test_data_dir+gen_dir+'/'+'test_inv_onlytrue_log_mj_trunc'+str(trunc)+str(n_samples)+'/'
