...
def load_data(path1,path2, n_events):
    ...
    extra=-1*np.ones((n_events,100-args.num_const,3))
    extra=torch.as_tensor(np.array(extra),dtype=torch.int64)
    x=torch.cat((x,extra),dim=1)
    
    extra_padding=np.zeros((n_events, 100-args.num_const), dtype=bool)    
    extra_padding=torch.as_tensor(np.array(extra_padding),dtype=torch.bool)
    padding_mask=torch.cat((padding_mask,extra_padding),dim=1)
    ...
    extra=-1*np.ones((n_events,100-args.num_const,3))
    extra=torch.as_tensor(np.array(extra),dtype=torch.int64)
    x1=torch.cat((x1,extra),dim=1)
                
    
    extra_padding=np.zeros((n_events, 100-args.num_const), dtype=bool)
    extra_padding=torch.as_tensor(np.array(extra_padding),dtype=torch.bool)
    padding_mask1=torch.cat((padding_mask1,extra_padding),dim=1)
